...
function countProfit(shoppers) {
    ...
    for (var i = 0; i < listBarang.length; i++) {
        ...
        for (var j = 0; j < shoppers.length; j++){
            if (shoppers[j].product === listBarang[i][0] && shoppers[j].amount <= objBarang.leftOver){
                objBarang.shoppers.push(shoppers[i].name);
                objBarang.leftOver -= shoppers[j].amount;
                objBarang.totalProfit += shoppers[j].amount * listBarang[i][1];
            }
        }
        result.push(objBarang);
    }
    return result;
}
